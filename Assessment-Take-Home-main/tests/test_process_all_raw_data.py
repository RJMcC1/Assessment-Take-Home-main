"""Tests for processing and combining all raw input CSV files."""
...
def test_get_raw_files_returns_sorted_matches(tmp_path: Path) -> None:
    """Raw file discovery should return sorted matching CSV paths."""
...
def test_process_all_raw_files_raises_when_no_raw_files(tmp_path: Path) -> None:
    """Processing should raise when no raw CSV files are found."""
...
def test_process_all_raw_files_combines_files_and_calls_normalizer(
    ...
    """Processing should combine inputs and pass them through normalizer."""
    project_root = tmp_path
