"""Tests for raw-data processing and normalization behavior."""
...
def test_clean_title_removes_bracketed_sections() -> None:
    """Titles should drop bracketed metadata and collapse extra spaces."""
...
def test_clean_title_handles_non_string_values() -> None:
    """Non-string values should produce an empty cleaned title."""
    assert clean_title(None) == ""
    assert clean_title(42) == ""


def test_normalize_dataframe_requires_expected_columns(author_lookup: dict[int, str]) -> None:
    """Normalization should fail fast when required columns are missing."""
...
def test_normalize_dataframe_cleans_and_sorts(author_lookup: dict[int, str]) -> None:
    """Normalization should clean values, enforce types, and sort by rating."""
    raw = pd.DataFrame(
        {
            "book_title": [
                "Alpha (Series #1)",
...
def test_raw_data_4_matches_expected_records(
    ...
    """RAW_DATA_4 should normalize to the expected logical output rows."""
